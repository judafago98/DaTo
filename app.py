...
# --- DISEÑO ULTRA PREMIUM Y RESPONSIVO (MÓVIL & PC) ---
st.markdown("""
    <style>
        ...
        @media (max-width: 768px) {
            ...
            [data-testid="stImage"] img { 
                max-height: 180px !important; /* Logo más pequeño en celular */
                border-radius: 15px !important;
            }
            [data-testid="stForm"] { 
                padding: 25px 20px !important; /* Formulario más compacto */
            }
            h1 { font-size: 2.2rem !important; }
            .kpi-card h1 { font-size: 2.5rem !important; } /* Textos grandes adaptados a la pantalla */
        }
    </style>
...
def color_estado(val):
    if val in ['Pagado', 'Pagada', 'Completado']: return 'background-color: rgba(16, 185, 129, 0.15); color: #34D399; font-weight: 600;'
    elif val in ['Activo', 'Pendiente']: return 'background-color: rgba(245, 158, 11, 0.15); color: #F59E0B; font-weight: 600;'
...
def color_estado_cuota(val):
    if 'Pagada' in val: return 'background-color: rgba(16, 185, 129, 0.1); color: #34D399; font-weight: 600;'
    elif 'Parcial' in val: return 'background-color: rgba(245, 158, 11, 0.1); color: #F59E0B; font-weight: 600;'
    else: return 'color: #F59E0B; font-weight: 500;'
...
# --- CONEXIÓN BLINDADA POR POOL DE CONEXIONES ---
@st.cache_resource
...
if not st.session_state['logeado']:
    ...
    with col_der:
        ...
        with st.form("form_login"):
            st.markdown("<h2 style='color: #00C6FF; font-weight: 600; margin-bottom: 5px; font-size: 2.2rem;'>Portal de Acceso</h2>", unsafe_allow_html=True)
            st.markdown("<p style='color: #94A3B8; margin-bottom: 25px; font-size: 1.05rem;'>Identifícate para entrar al ecosistema operativo.</p>", unsafe_allow_html=True)
            ...
            if st.form_submit_button("Ingresar al Sistema", use_container_width=True):
                try:
                    ...
                    if usuario_db:
                        st.session_state['logeado'], st.session_state['id_usuario'], st.session_state['nombre_usuario'], st.session_state['rol'] = True, usuario_db['id_usuario'], usuario_db['nombre_completo'], usuario_db['rol']
                        st.rerun()
...
# ==========================================
    ...
    st.sidebar.markdown(f"""
        <div style='padding: 12px; background: rgba(0, 198, 255, 0.03); border-radius: 12px; border: 1px solid rgba(0,198,255,0.1); margin-bottom: 15px; text-align: center;'>
            ...
            <span style='color:#00C6FF; font-size: 11px; font-weight: 500; letter-spacing: 1px;'>{str(st.session_state['rol']).upper()}</span>
        </div>
    """, unsafe_allow_html=True)
    ...
    if menu_seleccionado == "inicio":
        ...
        with tab_paz:
            ...
            if not creditos_act: st.info("Ningún cliente debe dinero actualmente.")
                ...
                if sel_paz:
                    ...
                    st.dataframe(df_plan.style.map(color_estado_cuota, subset=['Estado Actual']), use_container_width=True)
                        ...
                        with c6:
                            if cond:
                                ...
                                if bolsa:
                                    ...
                                    if st.button("💾 Guardar y Asentar en Inventario"):
        ...
        with tab_inv3:
            ...
            if not df_hist.empty:
                ...
                st.dataframe(df_hist.style.map(color_estado, subset=['Estado Físico']), use_container_width=True)
        ...
        with st.form("f_cli"):
            ...
            if st.form_submit_button("Crear Titular en la Base de Datos"):
                if doc and nom:
                    try:
        ...
        if not clientes or not inventario: st.warning("Se requiere registrar al menos un Cliente y tener Equipos Disponibles en Inventario.")
            ...
            if tipo_v:
                ...
                if cli_sel and eq_sel:
                    ...
                    st.divider()
                    if st.button("🚀 Procesar Originación y Archivar"):
                        valido = True
        ...
        if not activos: st.info("No hay carteras activas pendientes de pago.")
            ...
            if sel_titular:
                ...
                with st.form("f_pago"):
                    ...
                    if st.form_submit_button("✅ Sellar Transacción de Caja"):
        ...
        if df.empty: st.info("Índice de cartera vencida en ceros.")
            ...
            st.dataframe(df.style.map(color_estado, subset=['Estatus']), use_container_width=True)
        ...
        with tab_v:
            ...
            if not df_cart.empty:
                ...
                st.dataframe(df_cart.style.map(color_estado, subset=['Calificación']).map(color_ganancia_real, subset=['GANANCIA NETA EN EFECTIVO']), use_container_width=True)
        ...
        with tab_r:
            ...
            with c1:
                st.markdown("<h4 style='color:#ef4444;'>🗑️ Anular un Recibo de Pago</h4>", unsafe_allow_html=True)
                ...
                if pagos_db:
                    ...
                    with st.form("f_anular_pago"):
                        ...
                        if st.form_submit_button("Anular y Descontar de Caja") and pago_sel:
            ...
            with c2:
                st.markdown("<h4 style='color:#f97316;'>🚨 Echar Atrás una Venta Entera</h4>", unsafe_allow_html=True)
                ...
                if creds_db:
                    ...
                    with st.form("f_anular_venta"):
                        ...
                        if st.form_submit_button("Eliminar Completamente") and cred_sel:
            ...
            with c3:
                st.markdown("<h4 style='color:#eab308;'>📦 Eliminar Hardware de Bodega</h4>", unsafe_allow_html=True)
                ...
                if inv_db:
                    ...
                    with st.form("f_anular_hardware"):
                        ...
                        if st.form_submit_button("Borrar y Reembolsar Costo") and inv_sel:
        ...
        with tab_gas:
            ...
            with st.form("f_g"):
                ...
                if st.form_submit_button("Asentar Gasto") and cred_cc:
        ...
        with tab_in:
            ...
            with st.form("f_f_in"):
                ...
                if st.form_submit_button("Contabilizar Fondeo"):
                    if prov and iny > 0:
        ...
        with tab_out:
            ...
            if deudas:
                ...
                with st.form("f_d_out"):
                    ...
                    if st.form_submit_button("Generar Dispersión Fiduciaria") and d_sel:
        ...
        with tab_graf:
            ...
            with col_g1:
                ...
                if recaudos_mes:
                    ...
                    st.bar_chart(df_chart_rec, color="#34D399")
            ...
            with col_g2:
                ...
                if ventas_mes:
                    ...
                    st.area_chart(df_chart_ven, color="#00C6FF")
        ...
        with tab_riesgo:
            ...
            with c_r2:
                ...
                if estados_credito:
                    ...
                    st.bar_chart(df_est, color="#F59E0B")
        ...
        with tab_eficiencia:
            ...
            with col_e2:
                ...
                if not df_egresos.empty:
                    ...
                    st.bar_chart(df_egresos, color="#F87171")
        ...
        with tab_c1:
            ...
            with col_u1:
                ...
                with st.form("f_newUser"):
                    ...
                    if st.form_submit_button("✅ Proveer Acceso"):
            ...
            with col_u2:
                ...
                with st.form("f_change_rol"):
                    ...
                    if users_db:
                        ...
                        if st.form_submit_button("Aplicar Parámetro") and u_rol and new_rol:
            ...
            with col_u3:
                ...
                with st.form("f_reset"):
                    if users_db:
                        ...
                        if st.form_submit_button("Sobreescribir") and u_reset and p_reset:
        ...
        with tab_c2:
            ...
            if role_sel:
                ...
                with st.form("form_permisos"):
                    ...
                    if st.form_submit_button("Ejecutar Sello de Permisos"):
        ...
        with tab_c3:
            ...
            with st.form("form_nuevo_rol"):
                ...
                if st.form_submit_button("Inyectar Clasificación"):
                    if nuevo_rol_nombre:
                        try:
