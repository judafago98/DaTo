        ...
        if menu_seleccionado == "inicio":
            ...
            cursor.execute("""
                SELECT 
                    ...
                    -- RECAUDO TOTAL Y SEPARADO (Dinero Real Entrante)
                    ...
                    -- COMPRAS Y BODEGA (Dinero Saliente a Proveedores)
            ...
            # --- 1. MATEMÁTICA DE TESORERÍA (FLUJO DE CAJA REAL) ---
            ...
            # --- 3. MATEMÁTICA PATRIMONIAL (VALOR EMPRESA) ---
            ...
            # MÓDULO 1: LA TESORERÍA (LUMINOSO, IMPACTANTE Y ELEGANTE)
            html_tesoreria = f"""<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 20px; margin-bottom: 25px;"><div style="background: #FFFFFF; padding: 40px 30px; border-radius: 20px; box-shadow: 0 10px 40px rgba(0,0,0,0.04); border: 1px solid #E2E8F0; position: relative; overflow: hidden; display: flex; flex-direction: column; justify-content: center; text-align: center;"><div style="position: absolute; top: 0; left: 0; right: 0; height: 6px; background: linear-gradient(90deg, #3B82F6, #10B981, #F43F5E);"></div><div style="color: #64748B; font-size: 14px; font-weight: 800; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 15px;">🏦 Efectivo Real en Bancos</div><div style="color: {c_caja_text}; font-size: 4.5rem; font-weight: 900; letter-spacing: -2px; line-height: 1; text-shadow: 0 4px 15px {c_caja_text}25;">{fmt_cop(liquidez_banco)}</div><div style="background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 50px; padding: 10px 25px; display: inline-block; margin: 25px auto 0 auto;"><span style="color: #475569; font-size: 13px; font-weight: 600;">Entradas: <b style="color: #10B981;">{fmt_cop(entradas_totales)}</b> &nbsp;|&nbsp; Salidas: <b style="color: #E11D48;">{fmt_cop(salidas_totales)}</b></span></div></div><div style="background: #FFFFFF; padding: 30px; border-radius: 20px; border: 1px solid #E2E8F0; box-shadow: 0 10px 40px rgba(0,0,0,0.02);"><div style="color: #0F172A; font-size: 16px; font-weight: 900; text-transform: uppercase; border-bottom: 2px solid #F1F5F9; padding-bottom: 12px; margin-bottom: 18px;">📋 Detalle Flujo de Caja</div><div style="display: flex; justify-content: space-between; margin-bottom: 10px;"><span style="color: #475569; font-size: 14px; font-weight: 600;">Capital Inicial</span><span style="color: #0F172A; font-size: 14px; font-weight: 800;">{fmt_cop(cap_ini)}</span></div><div style="display: flex; justify-content: space-between; margin-bottom: 10px;"><span style="color: #475569; font-size: 14px; font-weight: 600;">Ingreso Total</span><span style="color: #0F172A; font-size: 14px; font-weight: 800;">{fmt_cop(recaudo_total)}</span></div><div style="border-top: 1px dashed #E2E8F0; margin: 15px 0;"></div><div style="display: flex; justify-content: space-between; margin-bottom: 10px;"><span style="color: #475569; font-size: 14px; font-weight: 600;">Compras Capital Inicial</span><span style="color: #0F172A; font-size: 14px; font-weight: 800;">{fmt_cop(compras_cap_ini)}</span></div><div style="display: flex; justify-content: space-between; margin-bottom: 10px;"><span style="color: #475569; font-size: 14px; font-weight: 600;">Recompras</span><span style="color: #0F172A; font-size: 14px; font-weight: 800;">{fmt_cop(recompras)}</span></div><div style="display: flex; justify-content: space-between; margin-bottom: 10px;"><span style="color: #475569; font-size: 14px; font-weight: 600;">Gastos Operativos (Fijos + Socios)</span><span style="color: #0F172A; font-size: 14px; font-weight: 800;">{fmt_cop(gastos_operativos_andres)}</span></div><div style="display: flex; justify-content: space-between; margin-bottom: 10px;"><span style="color: #475569; font-size: 14px; font-weight: 600;">Comisiones</span><span style="color: #0F172A; font-size: 14px; font-weight: 800;">{fmt_cop(g_comis_pagadas)}</span></div><div style="border-top: 2px solid #0F172A; margin: 15px 0;"></div><div style="display: flex; justify-content: space-between; align-items: center; background: {c_caja_bg}; padding: 15px; border-radius: 12px; border: 1px solid {c_caja_border};"><span style="color: {c_caja_text}; font-size: 15px; font-weight: 900; text-transform: uppercase;">Resultado</span><span style="color: {c_caja_text}; font-size: 20px; font-weight: 900;">{fmt_cop(liquidez_banco)}</span></div></div></div>"""
            st.markdown(html_tesoreria, unsafe_allow_html=True)

            # MÓDULO 2: DESGLOSE DE RECAUDO (Dato vs Fondo)
            html_split = f"""<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 25px;"><div style="background: #FFFFFF; border: 1px solid #E2E8F0; padding: 25px; border-radius: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.03); border-left: 6px solid #3B82F6;"><div style="display: flex; justify-content: space-between; align-items: center;"><div style="color: #1E293B; font-size: 18px; font-weight: 900;">Ingresos Cartera DaTo</div><div style="background: #EFF6FF; color: #2563EB; padding: 5px 12px; border-radius: 8px; font-size: 12px; font-weight: 800;">Propia</div></div><div style="color: #0F172A; font-size: 2.2rem; font-weight: 900; margin: 15px 0 5px 0;">{fmt_cop(recaudo_dato)}</div><div style="color: #64748B; font-size: 13px;">Abonos, Cuotas mensuales y Retomas.</div></div><div style="background: #FFFFFF; border: 1px solid #E2E8F0; padding: 25px; border-radius: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.03); border-left: 6px solid #8B5CF6;"><div style="display: flex; justify-content: space-between; align-items: center;"><div style="color: #1E293B; font-size: 18px; font-weight: 900;">Ingresos Fondos Externos</div><div style="background: #F5F3FF; color: #7C3AED; padding: 5px 12px; border-radius: 8px; font-size: 12px; font-weight: 800;">Terceros</div></div><div style="color: #0F172A; font-size: 2.2rem; font-weight: 900; margin: 15px 0 5px 0;">{fmt_cop(recaudo_fondo)}</div><div style="color: #64748B; font-size: 13px;">Desembolsos por venta de cartera de contado.</div></div></div>"""
            st.markdown(html_split, unsafe_allow_html=True)

            # MÓDULO 3: ESTADO DE RESULTADOS (P&G Detallado - SOLO DATO)
            html_pg = f"""<div style="background: #FFFFFF; padding: 30px; border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.04); margin-bottom: 30px; border: 1px solid #E2E8F0;"><div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 25px; border-bottom: 2px solid #F1F5F9; padding-bottom: 15px;"><div><div style="margin: 0; color: #0F172A; font-size: 22px; font-weight: 900; letter-spacing: -0.5px;">📈 ESTADO DE RESULTADOS (P&G DaTo)</div><div style="margin: 5px 0 0 0; color: #64748B; font-size: 13px;">Rentabilidad y gastos de la operación propia (Excluye ventas al fondo).</div></div><div style="text-align: right; background: #F0FDF4; padding: 10px 20px; border-radius: 12px; border: 1px solid #A7F3D0;"><div style="margin:0 0 5px 0; color:#047857; font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px;">ROI Histórico</div><div style="margin:0; color:#10B981; font-size: 1.8rem; font-weight: 900;">{roi_porcentaje:.2f}%</div></div></div><div style="display: flex; gap: 20px; flex-wrap: wrap;"><div style="flex: 1; min-width: 300px; background: #F8FAFC; border: 1px solid #E2E8F0; padding: 25px; border-radius: 12px; border-left: 6px solid #10B981;"><div style="margin: 0; color: #64748B; font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px;">(+) Ganancia Bruta DaTo</div><div style="margin: 10px 0 15px 0; color: #0F172A; font-size: 2.8rem; font-weight: 900; letter-spacing: -1px;">{fmt_cop(ganancia_bruta)}</div><div style="background: #FFFFFF; padding: 15px; border-radius: 8px; border: 1px solid #F1F5F9;"><div style="display: flex; justify-content: space-between; margin-bottom: 8px; border-bottom: 1px dashed #E2E8F0; padding-bottom: 8px;"><span style="color: #64748B; font-size: 13px;">Utilidad Comercial (Equipos Propios):</span><b style="color: #0F172A; font-size: 13px;">{fmt_cop(margen_comercial)}</b></div><div style="display: flex; justify-content: space-between;"><span style="color: #64748B; font-size: 13px;">Utilidad Financiera (Intereses Propios):</span><b style="color: #0F172A; font-size: 13px;">{fmt_cop(margen_intereses)}</b></div></div></div><div style="flex: 1; min-width: 300px; background: #FFFFFF; border: 1px solid #BFDBFE; padding: 25px; border-radius: 12px; box-shadow: 0 10px 25px rgba(37,99,235,0.05); border-left: 6px solid #3B82F6;"><div style="margin: 0; color: #1D4ED8; font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px;">(=) UTILIDAD NETA (Ganancia Libre)</div><div style="margin: 10px 0 15px 0; color: {c_util_text}; font-size: 2.8rem; font-weight: 900; letter-spacing: -1px;">{fmt_cop(utilidad_neta)}</div><div style="background: #FFF1F2; padding: 15px; border-radius: 8px; border: 1px solid #FECACA;"><div style="margin: 0 0 10px 0; color: #BE123C; font-size: 11px; font-weight: 800; text-transform: uppercase;">(-) Desglose de Gastos Reales ({fmt_cop(gastos_operativos_pg)}):</div><div style="display: flex; justify-content: space-between; margin-bottom: 8px; border-bottom: 1px dashed #FECACA; padding-bottom: 8px;"><span style="color: #9F1239; font-size: 12px;">Gastos Fijos (Operación):</span><b style="color: #BE123C; font-size: 12px;">{fmt_cop(g_fijos)}</b></div><div style="display: flex; justify-content: space-between; margin-bottom: 8px; border-bottom: 1px dashed #FECACA; padding-bottom: 8px;"><span style="color: #9F1239; font-size: 12px;">Comisiones de Ventas:</span><b style="color: #BE123C; font-size: 12px;">{fmt_cop(g_comis_pagadas)}</b></div><div style="display: flex; justify-content: space-between;"><span style="color: #9F1239; font-size: 12px;">Costo Fondeo (Intereses a Socios):</span><b style="color: #BE123C; font-size: 12px;">{fmt_cop(g_socios_pg)}</b></div></div></div></div></div>"""
            st.markdown(html_pg, unsafe_allow_html=True)

            # MÓDULO 4: PATRIMONIO Y RADIOGRAFÍA (Dónde está la plata)
            st.markdown("<div style='color: #0F172A; margin: 0 0 15px 0; font-size: 20px; font-weight: 800; border-bottom: 2px solid #E2E8F0; padding-bottom:10px;'>💎 Valor de la Empresa y Activos</div>", unsafe_allow_html=True)
            
            html_patrimonio = f"""<div style="background: #F8FAFC; border: 1px solid #E2E8F0; padding: 25px; border-radius: 16px; margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between;"><div><div style="margin:0; color:#475569; font-size:12px; font-weight:800; text-transform:uppercase; letter-spacing:1px;">Patrimonio Neto (Valor Total)</div><div style="margin:5px 0; color:#0F172A; font-size:2.5rem; font-weight:900;">{fmt_cop(patrimonio_neto)}</div><div style="margin:0; color:#64748B; font-size:13px;">Bancos + Bodega + Cartera + Cadenas - Pasivos</div></div><div style="font-size: 40px;">🏢</div></div>"""
            st.markdown(html_patrimonio, unsafe_allow_html=True)

            html_radiografia = f"""<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 35px;"><div style="background: #FFFFFF; border: 1px solid #E2E8F0; padding: 20px; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.02);"><div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 5px;"><span style="font-size:11px; color:#64748B; font-weight:800; text-transform:uppercase;">Inventario en Bodega</span><span>📦</span></div><div style="color:#0F172A; font-size:1.6rem; font-weight: 800;">{fmt_cop(bodega)}</div></div><div style="background: #FFFFFF; border: 1px solid #E2E8F0; padding: 20px; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.02);"><div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 5px;"><span style="font-size:11px; color:#64748B; font-weight:800; text-transform:uppercase;">Plata en la Calle (Activo)</span><span>🤝</span></div><div style="color:#0F172A; font-size:1.6rem; font-weight: 800;">{fmt_cop(cartera_total)}</div></div><div style="background: #FFFFFF; border: 1px solid #E2E8F0; padding: 20px; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.02);"><div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 5px;"><span style="font-size:11px; color:#64748B; font-weight:800; text-transform:uppercase;">Ahorro Cadenas (Intocable)</span><span>💰</span></div><div style="color:#0F172A; font-size:1.6rem; font-weight: 800;">{fmt_cop(ahorro_cadenas)}</div></div><div style="background: #FFF1F2; border: 1px solid #FECACA; padding: 20px; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.02);"><div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 5px;"><span style="font-size:11px; color:#BE123C; font-weight:800; text-transform:uppercase;">Mora > 30 Días</span><span>🚨</span></div><div style="color:#E11D48; font-size:1.6rem; font-weight: 800;">{riesgo_mora} Clientes</div></div></div>"""
